...
function<bool(ipt_pos, uint64_t& /*tsc*/, uint64_t& /*next*/)> get_tscs;
...
public:
    ...
    void output_packet(uint8_t packet[])
    {
        ...
        uint64_t tsc_delta;
        if (next_tsc < tsc || next_tsc == MAX_TSC_VAL)
            tsc_delta = 0;
        else
            tsc_delta = next_tsc-tsc;
        printf("%012lx:%06lx|", tsc, tsc_delta);
...
{
    if (argc != 3) {
        printf("Usage: %s [ipt-file] [sideband-file]\n", argv[0]);
        exit(EXIT_FAILURE);
    }

    shared_ptr<tsc_heuristics> tscs = make_shared<tsc_heuristics>(argv[2]);
    if (!tscs->parse_ipt(argv[1])) {
    ...
    get_tscs =
        [tscs](ipt_pos pos, uint64_t& tsc, uint64_t& next_tsc) {
    ...
    if (argc == 3 && !parser.input().open(argv[1])) {
